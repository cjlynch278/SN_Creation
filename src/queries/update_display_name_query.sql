...
select Name system_name, is_current, SN_System_ID from servicenow.servicenow_cmbd_ci_business_app where is_current = 1 and name is not null
union
select name system_name, is_current,SN_System_ID from servicenow.servicenow_cmbd_ci_service where is_current = 1 and name is not null
)
systems
